...
# from itertools import izip # no need to import for python 3
...
def make_hash(password):
    ...
    return 'PBKDF2${}${}${}${}'.format(
        ...
        COST_FACTOR,
        salt.decode("utf-8"),
        b64encode(pbkdf2(getattr(hashlib, HASH_FUNCTION), password, salt, COST_FACTOR, KEY_LENGTH,
                            )).decode("utf-8"))
...
def check_hash(password, hash_):
    ...
    hash_b = pbkdf2(getattr(hashlib, hash_function), password, salt, int(cost_factor), len(hash_a),
                        )
